...
export function ReportsPage() {
  ...
  // D. Individual Report: statement for a single entity by type + name/unique number + date range
  const individualReport = useMemo(() => {
    ...
    // Start with all bills of this type
    ...
    // Filter by customer name or unique number
    ...
    // Build statement rows for each entity
          ...
          <div className="rounded-xl bg-white dark:bg-muted border border-border overflow-hidden">
            <div className="p-4 border-b border-border flex items-center justify-between cursor-pointer" onClick={() => setShowFilters(!showFilters)}>
              <div className="flex items-center gap-2">
                ...
                {(revenueTypeFilter !== 'Business' || filterCustomer || filterUniqueNo || filterDateFrom || filterDateTo || filterLocality || filterRevenueItem) && (
            ...
            {showFilters && (
              <div className="p-4 space-y-0">
                ...
                <div className="flex justify-end pt-3">
                  <button onClick={() => { setRevenueTypeFilter('Business'); setFilterCustomer(''); setFilterUniqueNo(''); setFilterDateFrom(''); setFilterDateTo(''); setFilterLocality(''); setFilterRevenueItem(''); }} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
          ...
          <div className="rounded-xl bg-white dark:bg-muted border border-border overflow-hidden">
            <div className="p-4 border-b border-border flex items-center justify-between cursor-pointer" onClick={() => setShowFilters(!showFilters)}>
              <div className="flex items-center gap-2">
                <Filter className="w-4 h-4 text-primary dark:text-primary" />
                <span className="text-sm font-semibold text-foreground">Report Filters</span>
                {(filterCustomer || filterUniqueNo || filterDateFrom || filterDateTo || filterLocality || filterRevenueItem) && (
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-primary text-white text-[10px] font-bold">!</span>
                )}
              </div>
              <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform ${showFilters ? 'rotate-180' : ''}`} />
            ...
            {showFilters && (
              <div className="p-4 space-y-0">
                ...
                {/* Clear button */}
                <div className="flex justify-end pt-3">
                  <button onClick={() => { setFilterCustomer(''); setFilterUniqueNo(''); setFilterDateFrom(''); setFilterDateTo(''); setFilterLocality(''); setFilterRevenueItem(''); }} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
                    <X className="w-3.5 h-3.5" /> Clear all filters
                  </button>
                </div>
              </div>
