content = r'''// ─── Business Operating Permit Certificate PDF Generator ────────────────────────// Matches the Kpando Municipal Assembly permit design templateimport PDFDocument from 'pdfkit';import path from 'path';import fs from 'fs';export interface CertificateData {
  businessName: string;
  ownerName: string;
  regNumber: string;
  businessType: string;
...
function drawBorder(doc: PDFKit.PDFDocument, pageW: number, pageH: number) {
  ...
  drawCorner(doc, m + 3, m + 3, cs, false, false, color);                  // top-left
  drawCorner(doc, pageW - m - 3, m + 3, cs, true, false, color);           // top-right
  drawCorner(doc, m + 3, pageH - m - 3, cs, false, true, color);           // bottom-left
  drawCorner(doc, pageW - m - 3, pageH - m - 3, cs, true, true, color);    // bottom-right
}
...
export function generateCertificatePDF(data: CertificateData): Promise<Buffer> {
  ...
  // Format reg number as the permit number
  ...
  const legalText1 = 'Issued under the Local Governance Act, 2016 (Act 936)';
  const legalText2 = 'Section 87(1) to operate a business within the';
  doc.text(legalText1, 80, curY, { width: pageW - 160, align: 'center' });
  curY += 16;
  doc.text(legalText2, 80, curY, { width: pageW - 160, align: 'center' });
  curY += 16;
...
function formatDateString(dateStr: string): string {
  try {
    ...
    const day = d.getDate();
    const month = d.toLocaleDateString('en-US', { month: 'upper-long' }) || d.toLocaleDateString('en-US', { month: 'long' }).toUpperCase();
...
with open('/home/z/my-project/src/lib/notifications/certificate-pdf.ts', 'w') as f:
    f.write(content)
print('certificate-pdf.ts written successfully')
